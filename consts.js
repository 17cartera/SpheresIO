...
function exportConsts(scope)
{
    ...
    scope.SPAWN_TIME = 1; //amount of seconds it takes to spawn 1 unit
    scope.UNITS_PER_LEVEL = 10; //population capacity granted for each level of node controlled
    scope.REINFORCEMENT_RATE = 2; //amount of reinforcements each team builds per second
    ...
    scope.MAX_RANGE = 750*scope.SIZE_SCALE;//maximum range of a movingUnit group (currently only used by AI)
    ...
    scope.MIN_NODES_TO_GENERATE = 300//400; //minimum amount of nodes generated
    scope.MAX_NODES_TO_GENERATE = 400//500; //maximum amount of nodes generated
    scope.FACTORIES_TO_GENERATE = 6; //amount of factories to place on the map
    scope.PORTALS_TO_GENERATE = 3; //amount of portals to place on the map
